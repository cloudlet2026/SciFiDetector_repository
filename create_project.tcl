...
# 说明：自动扫描当前目录下的 sources_1、constrs_1、sim_1 文件夹，
#       将所有文件复制到新建工程内部，然后使用 add_files 添加，实现完全独立。
# ============================================================================

# 设置工程名和器件
set project_name "SciFiDetector"
...
# 复制所有 IP 核到工程内
set src_ip_dir "${origin_dir}/sources_1/ip"
if {[file exists $src_ip_dir]} {
    set dest_ip_parent "${proj_dir}/${project_name}.srcs/sources_1/"
    file mkdir $dest_ip_parent
    file copy -force $src_ip_dir $dest_ip_parent
    # 设置 IP 仓库路径为 sources_1/ip
    set_property ip_repo_paths "${dest_ip_parent}/ip" [current_fileset]
    update_ip_catalog -rebuild
    # 添加所有 .xci 文件到 sources_1
    set xci_files [glob -nocomplain -type f -directory "${dest_ip_parent}/ip" "*.xci"]
    foreach f $xci_files {
...
# 复制所有源码文件到工程内
set src_src_dir "${origin_dir}/sources_1/src"
if {[file exists $src_src_dir]} {
    set dest_src_parent "${proj_dir}/${project_name}.srcs/sources_1/"
    file mkdir $dest_src_parent
    file copy -force $src_src_dir $dest_src_parent
    # 添加所有 .v 文件
    set src_files [glob -nocomplain -type f -directory "${dest_src_parent}/src" "*.v" "*.vh" "*.vhd" "*.vhdl"]
    foreach f $src_files {
        add_files -fileset sources_1 $f
    }
...
# 复制 BD 目录并处理
set bd_name ""
set src_bd_dir "${origin_dir}/sources_1/bd"
if {[file exists $src_bd_dir]} {
    set dest_bd_parent "${proj_dir}/${project_name}.srcs/sources_1/"
    file mkdir $dest_bd_parent
    file copy -force $src_bd_dir $dest_bd_parent

    # 查找 .bd 文件
    set bd_files [glob -nocomplain -type f -directory "${dest_bd_parent}/bd" "*.bd"]
    ...
    if {[llength $bd_files] > 0} {
        ...
        puts "Warning: No .bd file found in ${dest_bd_parent}/bd"
    }

    # 如果存在对应的重建 Tcl 脚本
    set bd_tcl_script "${dest_bd_parent}/bd/${bd_name}.tcl"
    ...
    # 生成 wrapper
    set wrapper_file "${dest_bd_parent}/bd/${bd_name}/hdl/${bd_name}_wrapper.v"
    if {[file exists $wrapper_file]} {
        # wrapper 已存在，直接添加
        add_files -fileset sources_1 $wrapper_file
    } else {
        # 尝试打开 BD 并生成 wrapper
        set bd_design_path "${dest_bd_parent}/bd/${bd_name}/${bd_name}.bd"
...
set src_constr_dir "${origin_dir}/constrs_1"
if {[file exists $src_constr_dir]} {
    set dest_constr_parent "${proj_dir}/${project_name}.srcs/"
    file mkdir $dest_constr_parent
    file copy -force $src_constr_dir $dest_constr_parent
    set xdc_files [glob -nocomplain -type f -directory "${dest_constr_parent}/constrs_1" "*.xdc"]
    foreach f $xdc_files {
...
set src_sim_dir "${origin_dir}/sim_1"
if {[file exists $src_sim_dir]} {
    set dest_sim_parent "${proj_dir}/${project_name}.srcs/"
    file mkdir $dest_sim_parent
    file copy -force $src_sim_dir $dest_sim_parent
    set sim_files [glob -nocomplain -type f -directory "${dest_sim_parent}/sim_1" "*.v" "*.vh" "*.vhd" "*.vhdl"]
    foreach f $sim_files {
